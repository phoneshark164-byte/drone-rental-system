            ...
            <tr v-for="record in records" :key="record.id">
              ...
              <td>
                <div class="balance-change">
                  <div class="before">前: ¥{{ record.balanceBefore || '0.00' }}</div>
                  <div class="after">后: ¥{{ record.balanceAfter || '0.00' }}</div>
                </div>
              </td>
              <td>
...
const fetchRecords = async () => {
  try {
    ...
    const res = await request.get('/admin/api/payment/list', { params })
...
const fetchStats = async () => {
  try {
    const res = await request.get('/admin/api/payment/stats')
    if (res.code === 200 && res.data) {
      stats.value = {
...
.status-0 { background: #dbeafe; color: #2563eb; }
