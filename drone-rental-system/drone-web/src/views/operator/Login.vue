<template>
  <div class="login-wrapper">
    ...
    <div class="carousel-section">
      <div id="operatorCarousel" class="carousel slide carousel-fade" data-bs-ride="carousel" data-bs-interval="4000">
        <div class="carousel-indicators">
          <button
            v-for="(slide, index) in slides"
            :key="index"
            type="button"
            :data-bs-target="'#operatorCarousel'"
    ...
    <div class="login-section">
      <div class="login-container">
        <div class="login-header">
          <div class="login-logo operator-logo">
            <i class="bi bi-gear-fill"></i>
          </div>
          <h1 class="login-title">运营员登录</h1>
          <p class="login-subtitle">无人机智能租赁系统 - 运营后台</p>
        </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      // 保存登录信息
      setLoginInfo('operator-token-' + Date.now(), 'operator', res.data)
...
onMounted(() => {
  // 初始化轮播图
  const carouselEl = document.getElementById('operatorCarousel')
  if (carouselEl) {
...
.carousel-overlay {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(135deg, rgba(5, 150, 105, 0.4) 0%, rgba(4, 120, 87, 0.3) 100%);
  z-index: 1;
}
